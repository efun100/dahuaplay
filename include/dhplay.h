...
typedef struct
{
	...
	int				width;		//子窗口宽
	int				height;		//子窗口高
	int				width_projection;//子窗口的投影面的宽高
	int				height_projection;
	int				theta;		//子窗口中心在球面坐标系下得theta
	int				alpha;		//子窗口中心在球面坐标系下得alpha
	int				len;		//子窗口中心距离球面坐标系下远点的距离，此距离要大于球半径	
}SUBWIN_INFO;
#endif
...
#ifdef FTRACK_MODE
	SUBWIN_INFO      *subWins;                 /*新增子窗口信息,内存由外部申请*/ 
#endif
	int              reserved[1];				/*保留字节*/
}FISHEYE_OPTPARAM;
