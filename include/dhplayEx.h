...
typedef struct
{
	int radius;      //原始鱼眼圆半径——输入为8192坐标系
	int originX;     //原始鱼眼圆圆心——输入为8192坐标
	int originY;
	int src_width;   //原始鱼眼圆图像宽，高
	int src_height;
	FISHEYE_MOUNTMODE mount_mode;     //安装模式
	FISHEYE_CALIBRATMODE calib_mode;  //矫正模式
	SUBWIN_INFO sub_win_info;         //矫正子窗口信息，该参数通过Fisheye_GetParam()接口可获取
}FISHRECTIFY_INFO;
#endif
...
//鱼眼新增模式功能开关
#define NEWFUNC_PHONE_MODEL 0x01     //开启手机模式
#define NEWFUNC_FTRACK_MODE_CR 0x02  //开启1+1模式和1模式
#define NEWFUNC_FTRACK_MODE_RT 0x06  //开启使用1矫正图跟踪结果转鱼眼坐标功能
...
//		 [in] funcType:   开启哪些功能，默认为0, 该接口针对新增进来的模式进行开启选择, NEWFUNC_PHONE_MODEL|NEWFUNC_FTRACK_MODE_CR|NEWFUNC_FTRACK_MODE_RT
...
// 返回: TRUE 成功 FALSE 失败
//------------------------------------------------------------------------
PLAYSDK_API BOOL CALLMETHOD PLAY_OutsideRender(LONG nPort, int nX, int nY, int nWidth, int nHeight);
